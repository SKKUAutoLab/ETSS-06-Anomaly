...
python -m src.models.create_summary_index --index_dim "$index_dim" --root_path "$root_path" --annotationfile_path "$annotationfile_path" --batch_size "$batch_size" --frame_interval "$frame_interval" --captions_dir "${captions_dir}" --output_dir "${output_dir}"
