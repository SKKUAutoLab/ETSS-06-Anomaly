...
torchrun --nproc_per_node 2 --nnodes 1 -m src.models.llm_anomaly_scorer --root_path "$root_path" --annotationfile_path "$annotationfile_path" --batch_size "$batch_size" --frame_interval "$frame_interval" --summary_prompt "$summary_prompt" --output_summary_dir "$output_summary_dir" --captions_dir "$captions_dir" --ckpt_dir libs/llama/llama-2-13b-chat/ --tokenizer_path libs/llama/tokenizer.model
torchrun --nproc_per_node 2 --nnodes 1 -m src.models.llm_anomaly_scorer --root_path "$root_path" --annotationfile_path "$annotationfile_path" --batch_size "$batch_size" --frame_interval "$frame_interval" --output_summary_dir "$output_summary_dir" --context_prompt "$context_prompt" --format_prompt "$format_prompt" --output_scores_dir "$output_scores_dir" --ckpt_dir libs/llama/llama-2-13b-chat/ --tokenizer_path libs/llama/tokenizer.model --score_summary
