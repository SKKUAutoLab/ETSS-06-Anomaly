...
for pretrained_model_name in "${pretrained_model_names[@]}"; do
    ...
    python -m src.models.image_captioner --root_path "$root_path" --annotationfile_path "$annotationfile_path" --batch_size "$batch_size" --frame_interval "$frame_interval" --pretrained_model_name "$pretrained_model_name" --output_dir "$output_dir"
done
