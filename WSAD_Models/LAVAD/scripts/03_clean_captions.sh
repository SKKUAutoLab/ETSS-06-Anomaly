...
python -m src.models.image_text_caption_cleaner --root_path "$root_path" --annotationfile_path "$annotationfile_path" --batch_size "$batch_size" --frame_interval "$frame_interval" --output_dir "$output_dir" --captions_dir_template "${captions_dir_template}" --index_dir "${index_dir}" --fps "$fps" --clip_duration "$clip_duration" --num_samples "$num_samples" --num_neighbors "$num_neighbors"
