...
def MSNSD(features,scores,bs,batch_size,drop_out,ncrops,k):
    ...
    if ncrops == 1:
        normal_features = features[0:batch_size]
        ...
        normal_features = features[0:batch_size * 10]
        normal_scores = scores[0:batch_size]
...
class Backbone(nn.Module):
    def __init__(self, *, dim, depth, heads, mgfn_type = 'gb', kernel = 5, dim_headnumber = 64, ff_repe = 4, dropout = 0., attention_dropout = 0.):
        super().__init__()
        ...
        for _ in range(depth):
            ...
            self.layers.append(nn.ModuleList([nn.Conv1d(dim, dim, 3, padding = 1), attention, FeedForward(dim, repe = ff_repe, dropout = dropout)]))
...
class mgfn(nn.Module):
    def __init__(self, *, classes=0, dims = (64, 128, 1024), depths = (args.depths1, args.depths2, args.depths3), mgfn_types = (args.mgfn_type1,args.mgfn_type2, args.mgfn_type3),
                 lokernel = 5, channels = 2048, ff_repe = 4, dim_head = 64, dropout = 0., attention_dropout = 0.):
        super().__init__()
        ...
        for ind, (depth, mgfn_types) in enumerate(zip(depths, mgfn_types)):
            ...
            self.stages.append(nn.ModuleList([Backbone(dim = stage_dim, depth = depth, heads = heads, mgfn_type = mgfn_types, ff_repe = ff_repe, dropout = dropout, attention_dropout = attention_dropout),
                                              nn.Sequential(LayerNorm(stage_dim), nn.Conv1d(stage_dim, dims[ind + 1], 1, stride = 1)) if not is_last else None]))
