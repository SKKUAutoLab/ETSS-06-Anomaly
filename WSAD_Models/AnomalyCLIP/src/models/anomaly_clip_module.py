...
from torchmetrics import AUROC, ROC, AveragePrecision, ConfusionMatrix, F1Score, MeanMetric, PrecisionRecallCurve
...
class AnomalyCLIPModule(LightningModule):
    def __init__(self, net: torch.nn.Module, optimizer: torch.optim.Optimizer, scheduler: torch.optim.lr_scheduler, loss: ComputeLoss, **kwargs):
        super().__init__()
        ...
        self.confmat = ConfusionMatrix(task="multiclass", num_classes=self.hparams.num_classes, normalize="true")
        self.pr = Precision(task="binary")
        self.top1_accuracy = Accuracy(task="multiclass", num_classes=self.hparams.num_classes, top_k=1, average=None)
        self.top5_accuracy = Accuracy(task="multiclass", num_classes=self.hparams.num_classes, top_k=5, average=None)
        self.mc_auroc = MulticlassAUROC(num_classes=self.hparams.num_classes, average=None, thresholds=None)
        self.mc_aupr = AveragePrecision(task="multiclass", num_classes=self.hparams.num_classes, average=None)
        self.labels = []
        self.abnormal_scores = []
        self.class_probs = []

    def forward(self, image_features: torch.Tensor, labels, ncentroid: torch.Tensor, segment_size: int = 1, test_mode: bool = False):
        return self.net(image_features, labels, ncentroid, segment_size, test_mode)
    ...
    def model_step(self, batch: Any):
        ...
        logits, logits_topk, scores, idx_topk_abn, idx_topk_nor, idx_bottomk_abn = self.forward(image_features, labels, ncentroid=self.ncentroid)
        return logits, logits_topk, labels, scores, idx_topk_abn, idx_topk_nor, idx_bottomk_abn

    def training_step(self, batch: Any, batch_idx: int):
        similarity, similarity_topk, labels, scores, idx_topk_abn, idx_topk_nor, idx_bottomk_abn = self.model_step(batch)
        loss, ldir_abn, ldir_nor, ltopk_abn, lbottomk_abn, ltopk_nor, lsmooth, lsparse = self.criterion(similarity, similarity_topk, labels, scores, idx_topk_abn, idx_topk_nor, idx_bottomk_abn)
        ...
        self.log("train/dir_abn_loss", self.dir_abn_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/dir_nor_loss", self.dir_nor_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/topk_abn_loss", self.topk_abn_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/bottomk_abn_loss", self.bottomk_abn_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/topk_nor_loss", self.topk_nor_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/smooth_loss", self.smooth_loss, on_step=False, on_epoch=True, prog_bar=True)
        self.log("train/sparse_loss", self.sparse_loss, on_step=False, on_epoch=True, prog_bar=True)
        return {"loss": loss}
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        ...
        similarity, abnormal_scores = self.forward(image_features, labels, self.ncentroid, segment_size, test_mode=True)
    ...
    def on_validation_epoch_end(self):
        ...
        class_probs = torch.cat((class_probs[:, :normal_idx], normal_probs, class_probs[:, normal_idx:]), dim=1)
        ...
        metrics = {"epoch": self.trainer.current_epoch, "auc_roc": auc_roc.item(), "auc_pr": auc_pr.item(), "mean_mc_auroc": mean_mc_auroc.item(), "mean_mc_aupr": mean_mc_aupr.item(),
                   "mc_auroc": mc_auroc.tolist(), "mc_aupr": mc_aupr.tolist(), "optimal_threshold": optimal_threshold.item()}
        save_dir = Path(self.hparams.save_dir)
    ...
    def on_test_start(self):
        ...
        save_dir = Path(os.path.join("logs/train/runs", str(save_dir)))
        ...
        if self.trainer.datamodule.hparams.visualize:
            self.visualizer = Visualizer(self.trainer.datamodule.hparams.normal_id, self.trainer.datamodule.hparams.labels_file, self.trainer.datamodule.hparams.image_tmpl, save_dir, self.device)
        else:
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        if self.visualizer is not None:
            self.visualizer.process_video(abnormal_scores, class_probs, softmax_similarity, labels, path)
        return {"abnormal_scores": abnormal_scores, "labels": labels, "class_probs": class_probs}
    ...
    def test_epoch_end(self, outputs: List[Any]):
        ...
        precision, recall, thresholds = self.pr_curve(abnormal_scores, labels_binary)
        auc_pr = self.average_precision(abnormal_scores, labels_binary)
        class_probs_without_normal = torch.cat((class_probs[:, :normal_idx], class_probs[:, normal_idx + 1 :]), dim=1)
        y_pred = []
        ...
        top5_preds = torch.where(y_pred.unsqueeze(1) == normal_idx, torch.cat((torch.tensor([normal_idx]).unsqueeze(0).expand(top5_preds.shape[0], -1).to(self.device), top5_preds[:, :4]), dim=1), top5_preds)
        for class_idx in range(num_classes):
            ...
            top5_accuracy[class_idx] = (top5_preds[class_mask] == class_labels.view(-1, 1)).any(dim=1).float().mean()
        mc_auroc = self.mc_auroc(class_probs, labels)
        mc_aupr = self.mc_aupr(class_probs, labels)
        mc_auroc_without_normal = torch.cat((mc_auroc[:normal_idx], mc_auroc[normal_idx + 1 :]))
        mc_auroc_without_normal[mc_auroc_without_normal == 0] = torch.nan
        ...
        ckpt_path = Path(self.trainer.ckpt_path)
        save_dir = os.path.normpath(ckpt_path.parent).split(os.path.sep)[-1]
        save_dir = Path(os.path.join("logs/eval/runs", str(save_dir)))
        if not save_dir.is_dir():
            save_dir.mkdir(parents=True, exist_ok=True)
        ...
        metrics = {"epoch": self.trainer.current_epoch, "auc_roc": auc_roc.item(), "auc_pr": auc_pr.item(), "mean_mc_auroc": mean_mc_auroc.item(), "mean_mc_aupr": mean_mc_aupr.item(),
                   "mc_auroc": mc_auroc.tolist(), "mc_aupr": mc_aupr.tolist(), "top1_accuracy": top1_accuracy.tolist(), "top5_accuracy": top5_accuracy.tolist(), "optimal_threshold": optimal_threshold.item()}
    ...
    def configure_optimizers(self):
        param_list = []
        param_list.append({"params": self.net.selector_model.parameters(), "lr": self.hparams.solver.lr * self.hparams.solver.selector_model_ratio, "name": "selector_model"})
        param_list.append({"params": self.net.temporal_model.parameters(), "lr": self.hparams.solver.lr * self.hparams.solver.temporal_model_ratio, "name": "temporal_model"})
        param_list.append({"params": self.net.prompt_learner.parameters(), "lr": self.hparams.solver.lr * self.hparams.solver.prompt_learner_ratio, "name": "prompt_learner"})
        param_list.append({"params": self.net.text_encoder.text_projection, "lr": self.hparams.solver.lr * self.hparams.solver.text_projection_ratio, "name": "text_projection"})
        optimizer = self.optimizer(params=param_list)
        if self.scheduler is not None:
            successor = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, float(self.trainer.max_epochs))
            scheduler = self.scheduler(optimizer=optimizer, successor=successor)
            return {"optimizer": optimizer, "lr_scheduler": {"scheduler": scheduler, "monitor": "train/loss", "interval": "epoch", "frequency": 1}}
