...
class Visualizer:
    ...
    def compute_predictions(self, abnormal_scores, class_probs, softmax_similarity, threshold=0.5):
        ...
        top3_preds = torch.where(y_pred.unsqueeze(1) == self.normal_idx, torch.cat((torch.tensor([self.normal_idx]).unsqueeze(0).expand(top3_preds.shape[0], -1).to(self.device), top3_preds[:, :2]), dim=1), top3_preds)
    ...
    def generate_video(self, figs, video_path, video_fps=30):
        ...
        video_writer = cv2.VideoWriter(str(video_path), cv2.VideoWriter_fourcc(*"mp4v"), video_fps, (int(fig_size[0]), int(fig_size[1])))
        for fig in figs:
            fig.canvas.draw()
    ...
    def create_figure(self, i, img, abnormal_scores, top3_preds, softmax_similarity, labels, class_names, title, threshold):
        ...
        if abnormal_scores[i] >= threshold:
            ...
            ax2.bar(highlight_idx, [softmax_similarity[idx] for idx in highlight_idx], color=highlight_color, align="center")
        ...
        if start_idx is not None:
            rect = plt.Rectangle((start_idx, ymin), labels.size(0) - start_idx, ymax - ymin, color="#e15759", alpha=0.5)
            ax3.add_patch(rect)
    ...
    def generate_figures(self, dir_path, abnormal_scores, top3_preds, softmax_similarity, labels, class_names, num_frames, title, threshold=0.5):
        figs = []
        for i in range(num_frames):
            img_path = os.path.join(dir_path, self.image_tmpl.format(i))
            img = cv2.imread(img_path)
            fig = self.create_figure(i, img, abnormal_scores, top3_preds[i], softmax_similarity[i], labels, class_names, title, threshold)
            figs.append(fig)
        return figs
    ...
    def process_video(self, abnormal_scores, class_probs, softmax_similarity, labels, path):
        ...
        class_names_except_normal = [name.replace("RoadAccidents", "RoadAcc.") for name in class_names_except_normal]
        title = os.path.splitext(path[0].split("/")[-1])[0]
        y_pred, top3_preds, top3_probs = self.compute_predictions(abnormal_scores, class_probs, softmax_similarity)
        figs = self.generate_figures(dir_path, abnormal_scores_np, top3_preds, softmax_similarity_np, labels, class_names_except_normal, len(abnormal_scores), title)
