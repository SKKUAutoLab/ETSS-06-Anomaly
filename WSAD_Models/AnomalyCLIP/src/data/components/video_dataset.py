...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    def _get(self, record: VideoRecord, frame_start_indices: "np.ndarray[int]") -> Union[Tuple[List[Image.Image], Union[int, List[int]]], Tuple["torch.Tensor[num_frames, channels, height, width]", Union[int, List[int]]], Tuple[Any, Union[int, List[int]]]]:
        if self.test_mode or self.val_mode:
            labels = list()
            ...
            if self.annotations:
                ...
                start_indices = []
                stop_indices = []
            for i in range(record.num_frames // self.ncrops):
        ...
        for start_index in frame_start_indices:
            for i in range(self.frames_per_segment):
                ...
                frame_index += record.start_frame
        ...
        if self.test_mode:
            ...
            return images, np.asarray(labels), record.label, segment_size, record.path
        elif self.val_mode:
