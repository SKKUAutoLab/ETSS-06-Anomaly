...
def main(args):
    ...
    for label_path in sorted(os.listdir(args.video_dir), reverse=True):
        print(f'Processing {label_path}...')
        label_save_dir = os.path.join(args.save_dir, label_path)
        if not os.path.isdir(label_save_dir):
            os.makedirs(label_save_dir)
        label_dir = os.path.join(args.video_dir, label_path)
        video_list = sorted(os.listdir(label_dir), key=lambda x: os.path.getsize(os.path.join(label_dir, x)), reverse=False)
        for video_name in tqdm(video_list):
            for crop_type in range(10):
                video_path = os.path.join(label_dir, video_name)
                save_path = os.path.join(label_save_dir, video_name)
                save_path = save_path.replace('.avi', f'__{str(crop_type)}.npy')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_dir', type=str, default='/media/Data1/shang_raw/videos')
    parser.add_argument('--save_dir', type=str, default='/media/Data1/shang_dataset/vitl/rgb')
