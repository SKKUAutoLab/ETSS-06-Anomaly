...
def mixup(label_img: torch.Tensor, label_onehot: torch.Tensor, unlab_img: torch.Tensor, unlabeled_pred: torch.Tensor, input_lablel_num, args):
    ...
    mixup_label = label_onehot * alpha.view(bn, 1) + unlabeled_pred * (1 - alpha).view(bn, 1)
...
def mixup1(label_img: torch.Tensor, label_onehot: torch.Tensor, unlab_img: torch.Tensor, unlabeled_pred: torch.Tensor, input_lablel_num, args):
    labeled_idx =  label_img.shape[0]
    unlabeled_idx = unlab_img.shape[0]
    if labeled_idx > unlabeled_idx:
        front_img = label_img[:input_lablel_num]
