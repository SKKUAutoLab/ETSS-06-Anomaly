...
def _check_reduction_params(reduction):
    assert reduction in ("mean","sum","none"), "reduction should be in ``'none'`` | ``'mean'`` | ``'sum'``. ``'none'``, given {}".format(reduction)
...
class JSD_div(nn.Module):
    ...
    def forward(self, *input: Tensor) -> Tensor:
        ...
        mean_entropy: Tensor = sum(list(map(lambda x: self._entropy_criterion(x), input))) / len(input)
