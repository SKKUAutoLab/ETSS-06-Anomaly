    ...
    def run(self):
        try:
            ...
            for idx, row in test_df.iterrows():
                ...
                self.log.emit(f"[{idx + 1}/{total}] {row['path']} | "
                              f"pred_time={prediction['accident_time']:.2f}s "
                              f"center=({prediction['center_x']:.3f}, {prediction['center_y']:.3f}) "
                              f"type={prediction['type']} conf={details['confidence']:.3f} | "
                              f"FPS={speed:.2f}")
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.image_label = ClickableVideoLabel("Click Run Testing to start")
        ...
        self.metrics_label = QtWidgets.QLabel("Metrics will appear after testing finishes.")
    ...
    def format_metrics(self, metrics: dict) -> str:
        order = ["n", "temporal_gaussian_sigma_2", "spatial_gaussian_sigma_0.1", "classification_accuracy", "harmonic_mean", "time_mae_seconds", "time_accuracy_at_0.5s", "time_accuracy_at_1s", "time_accuracy_at_2s"]
        return "    ".join(f"{key}: {float(metrics[key]):.4f}" for key in order if key in metrics)
