# yolo.py
# Drop-in replacement for fasterrcnn.py using YOLOv8 for detection and ResNet101 for feature extraction
# Outputs the same format: res_feat (num_objs+1, 2048) numpy array, res_dect (num_objs+1, 6) numpy array
...
class FeatureExtractor(nn.Module):
    def __init__(self):
        ...
        modules = list(resnet.children())[:-1]  # Remove the last FC layer, keep avgpool
        self.resnet = nn.Sequential(*modules)

    def forward(self, x):
        return self.resnet(x).squeeze()  # (2048,)
...
class FasterRCNN(object):
    def __init__(self, arg=None):
        ...
        # COCO: 0=person, 1=bicycle (bike), 2=car, 3=motorcycle (motorbike), 5=bus
        self.class_map = {0: 0, 1: 1, 3: 2, 2: 3, 5: 4}

        print('Loaded YOLOv8 and ResNet101 for detection and features.')
