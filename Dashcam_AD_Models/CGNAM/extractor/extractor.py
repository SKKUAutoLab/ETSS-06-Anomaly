...
class Extractor(object):
    def __init__(self, det_vis = False, write_video = False):
    ...
    def open(self, video_path):
        ...
        if self.write_video:
            ...
            fourcc =  FOURCC(*'mp4v')
            self.det_output = cv2.VideoWriter("output/det_{:s}.avi".format(video_id), fourcc, 20, (self.im_width,self.im_height))
...
if __name__ == "__main__":
    ...
    for _split in [_train, _test]:
        ...
        for sample in all_name:
            ...
            num_obj, det, feat, ffeat = extractor.extract()
            ext_toc = time.time()
            # n v t c -> n c t v m
            # data = np.transpose(data,(0,3,2,1))
            # data = np.expand_dims(data,axis=-1)
