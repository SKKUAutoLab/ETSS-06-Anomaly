...
print("train/validation/test dataset size", len(dataset_train), len(dataset_val), len(dataset_test))
arch_name = p['architecture_type']
expdir = '%s-%s-batch%d-layer%d-embed-%d' % (arch_name, p['feature'], p['batch_size'], p['num_layers'], p['hidden_size'])
        ...
        if((epoch % args.display_period == 0) and (epoch != 0)):
            ...
            if(p['prediction_type'] == 'both'):
                print('[both] train/val/val max acc tIoU@0.5 : %.4f / %.4f / %.4f' % ( (stats['cause-iou-thr-train'][4]+stats['effect-iou-thr-train'][4])/2,
                     (stats['cause-iou-thr-val'][4]+stats['effect-iou-thr-val'][4])/2, (stats['max-cause-iou-thr-val'][4]+stats['max-effect-iou-thr-val'][4])/2))
...
if(p['prediction_type'] == 'both'):
    ...
    if(args.num_experiments > 1):
        ...
        print("%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f" % (float(torch.mean(cause_thr_test[:, 4])), float(torch.std(cause_thr_test[:, 4])), float(torch.mean(effect_thr_test[:, 4])),
                                                      float(torch.std(effect_thr_test[:, 4])), float(torch.mean(both_thr_test[:, 4])), float(torch.std(both_thr_test[:, 4]))))
    else:
        print("cause/effect/both test max performance mean @ IoU=0.5")
        print("%.4f\t%.4f\t%.4f" % (float(torch.mean(cause_thr_test[:, 4])), float(torch.mean(effect_thr_test[:, 4])), float(torch.mean(both_thr_test[:, 4]))))
