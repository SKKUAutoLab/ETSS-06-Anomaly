...
def get_frames_from_path_list(path_list):
    ...
    for path in path_list:
        img = cv2.imread(path)
        # default: img = cv2.resize(img, [512, 320])
...
def evaluate_vids(vid_root, samples=200, downsample=False, num_frames=25):
    ...
    # default: all_gt = np.zeros((samples, num_frames, 3, 320, 512))
    # default: all_gen = np.zeros((samples, num_frames, 3, 320, 512))
    all_gt = np.zeros((samples, num_frames, 3, 256, 256))
    all_gen = np.zeros((samples, num_frames, 3, 256, 256))
