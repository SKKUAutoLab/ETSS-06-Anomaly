...
parser.add_argument('--trainfile_dota', type=str,
                    default="data/DoTA/train_split.txt")
parser.add_argument('--testfile_dota', type=str,
                    default="data/DoTA/val_split.txt")
...
def validate(args, val_loader, model, dataset='KITTI'):
    ...
    if dataset == 'DoTA':
        ...
        gt = joblib.load(
            open(os.path.join('data/DoTA', "ground_truth_demo/gt_label.json"),
        ...
        joblib.dump(scores, os.path.join(paths['log_dir'], "frame_scores_%s_%s_dada.json" % (
            args.height, args.width)))
        gt = joblib.load(
            open(os.path.join('data/DADA2000', "ground_truth_demo/gt_label.json"),
