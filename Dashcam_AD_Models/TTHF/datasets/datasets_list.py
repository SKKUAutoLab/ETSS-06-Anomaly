...
class DoTADatasetSubMPM(data.Dataset):
    def __init__(self, args, train=True, return_filename=False):
        ...
        self.args = args
        self.data_path = "data/DoTA/frames"  # path to the video frames of DoTA dataset
        if train is True:
            self.label_file = 'data/DoTA/metadata_train.json'  # path to gt
            self.one_hot_file_M = 'data/DoTA/ground_truth_demo/train_one_hot_label.json'  # path to label
            self.one_hot_file_S = 'data/DoTA/ground_truth_demo/train_one_hot_label_bi.json'
            self.one_hot_label_M = joblib.load(open(self.one_hot_file_M, 'rb'))
            self.one_hot_label_S = joblib.load(open(self.one_hot_file_S, 'rb'))
        else:
            self.label_file = 'data/DoTA/metadata_val.json'
...
class DADADatasetSubMPM(data.Dataset):
    def __init__(self, args, train=True):
        ...
        for video in sorted(self.fileset):
            ...
            self.videos[video_name]['frame'] = glob.glob(
                os.path.join(self.data_path, video_path[0], video_path[1], 'images',
                             '*' + '.png'))
            self.videos[video_name]['frame'].sort()
            video_frame = self.videos[video_name]['frame']
